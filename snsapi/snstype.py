...
class CommentID(utils.JsonDict):
    """
    All information to locate one comment is here.
    ...
    There are three mandatory fields:
       ...
       * channel: Name of the instantiated channel
         (e.g. 'renren_account_1').
         Same as a channel's ``.jsonconf['channel_name']``.
       * status_id: Id of the status which this comment pertains to..


    **NOTE**: This object is mainly for SNSAPI to identify a Comment.
    Upper layer had better not to reference fields of this object directly.
    If you must reference this object, please do not touch those
    non-mandatory fields.

    """
    def __init__(self, platform=None, channel=None, status_id=None):
...
class Message(utils.JsonDict):
    ...
    Data Fields:
       ...
       * ``platform``: a string describing the platform
         where this message comes from. See 'snsapi/platform.py'
         for more details.
       * ``raw``: the raw json or XML object returned from
         the platform specific API. This member is here to give
         upper layer developers the last chance of manipulating
         any available information. Having an understanding of
         the platform-specific returning format is essential.
       ...
       * ``ID``: a ``MessageID`` object. This ID should be enough
         to identify a message across all different platforms.
    ...
    Optional fields of 'parsed' are:
       ...
       * ``body``: a string. The 'content' of RSS, the 'body' of HTML,
         or whatever semantically meaning the body of a document.
       ...
       * ``text_trace``: a string. Using any (can be platform-specific)
         method to construct the trace of this message. e.g.
    ...
    def dump(self, tz=None):
        ...
        This level targets console output. It only digests essential
    ...
    def dump_full(self):
        ...
        refer to the official API web page for more info.
        '''
        return self._dumps()
    ...
    def digest(self):
        ...
        for those applications requires message de-duplication.
...
class Comment(utils.JsonDict):
    '''
    The Comment base class for SNSAPI

    Data Fields:

       * ``platform``: a string describing the platform
         where this comment comes from. See 'snsapi/platform.py'
         for more details.
       * ``raw``: the raw json or XML object returned from
         the platform specific API. This member is here to give
         upper layer developers the last chance of manipulating
         ...
         the platform-specific returning format is essential.
       * ``parsed``: this member abstracts some common fields
         that all comments are supposed to have. e.g. 'username',
         'time', 'text', etc.
       * ``ID``: a ``CommentID`` object. This ID should be enough
         to identify a comment across all different platforms.

    For details of ``ID``, please see the docstring of ``CommentID``.
    ...
    Optional fields of 'parsed' are:
       ...
       * ``deleted``: Bool. For some OSN.
       * ``description``: a string. Sharing channel of some OSN; etc.
       * ``text_orig``: a string. The original text, also known as
         "root comment" in some context. e.g. the earliest status
         in one thread.
       * ``text_last``: a string. The latest text, also known as
         "comment" in some context. e.g. the reply or forwarding
         comments made by the last user.
       * ``text_trace``: a string. Using any (can be platform-specific)
         method to construct the trace of this comment. e.g.
         the forwarding / retweeting / reposting sequence.
         There is no unified format yet.
       * ``username_origin``: a string. The username who posts 'text_orig'.
    ...
    def __init__(self, dct=None, platform=None, channel=None, status_id=None, conf={}):

        self.conf = conf
        self['deleted'] = False
        self['ID'] = CommentID(platform, channel, status_id)
    ...
    def dump(self, tz=None):
        ...
        To control the format, please rewrite dump() in derived Comment class.
...
class CommentList(list):
    ...
    def append(self, e):
        if isinstance(e, Comment):
            if hasattr(e, 'deleted') and e.deleted:
                logger.debug("Trying to append Deleted Comment type element. Ignored")
            else:
                super(CommentList, self).append(e)
        else:
            logger.debug("Trying to append non-Comment type element. Ignored")

    def extend(self, l):
        if isinstance(l, CommentList):
            super(CommentList, self).extend(l)
        elif isinstance(l, list):
            # We still extend the list if the user asks to.
            # However, a warning will be placed. Doing this
            # may violate some properties of CommentList, e.g.
            # there is no Deleted Message in the list.
            super(CommentList, self).extend(l)
            logger.warning("Extend CommentList with non CommentList list.")
        else:
            logger.warning("Extend CommentList with unknown type.")
